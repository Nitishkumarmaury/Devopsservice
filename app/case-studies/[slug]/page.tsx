...
export default async function CaseStudyDetailPage({ params }: CaseStudyPageProps) {
 ...
 <p className="font-mono text-xs font-semibold uppercase tracking-[0.18em] text-brand">Technologies</p>
 ...
 <StaggerReveal className="grid items-stretch gap-5">
 {sections.map(([title, text]) => (
 <article key={title} className="flex h-full flex-col border border-border bg-canvas-soft p-6">
 <h2 className="text-2xl font-semibold tracking-[-0.025em] text-ink">{title}</h2>
 <p className="mt-3 text-base leading-8 text-ink-secondary">{text}</p>
 </article>
 ))}
 </StaggerReveal>
 </div>
