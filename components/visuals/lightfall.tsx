...
const prepColors = (input?: string[]) => {
  const base = (input?.length ? input : ["#A6C8FF", "#8FB6E8", "#F2C7FF"]).slice(0, MAX_COLORS);
  const arr: RGB[] = [];
...
export default function Lightfall({
  ...
  useEffect(() => {
    ...
    if (!container || !supportsWebGl()) {
      if (container) container.dataset.webgl = "unavailable";
      return;
    }
    ...
    try {
      ...
      renderer = new Renderer({ dpr: pixelRatio, alpha: true, antialias: true });
  ...
  return (
    <div
      ref={containerRef}
      className={["lightfall-container", className].filter(Boolean).join(" ")}
      style={mixBlendMode ? { mixBlendMode } : undefined}
    >
      <div className="lightfall-fallback" aria-hidden="true" />
    </div>
  );
}
