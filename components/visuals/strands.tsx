...
const buildPalette = (colors: string[]) => {
  const filled = colors.length ? colors.slice(0, MAX_COLORS) : ["#ffffff"];
  const padded: number[][] = [];
...
export default function Strands({
  ...
  return (
    <div ref={containerRef} className={`strands-container ${className}`.trim()} style={style}>
      <div className="strands-fallback" aria-hidden="true" />
    </div>
  );
}
